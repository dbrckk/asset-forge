...
## Current priority
- Consider additional engine adapters and higher-level scene systems; runtime-atlas now includes a persistent 2D camera with top-left world position, zoom, per-entity X/Y parallax factors, per-call camera overrides, and camera-aware cached batch variants.
...
## Last verified
- 2026-09-19: latest GitHub source inspected after adding applyCameraToSpriteInstances(), camera x/y/zoom validation, per-entity parallaxX/parallaxY, canvas-runtime camera/setCamera(), per-call camera overrides, pre-culling world-to-screen transformation, and camera-aware entity batch cache variants.
