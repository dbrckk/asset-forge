...
## Current priority
- Consider additional engine adapters and higher-level scene systems; runtime-atlas now includes world↔screen coordinate conversion and rotation/pivot-aware entity hit testing/picking on the same camera/layer/hierarchy transforms used for rendering.
...
## Last verified
- 2026-09-19: latest GitHub source inspected after adding worldToScreenPoint()/screenToWorldPoint(), pointHitsSpriteInstance() with logical or trimmed-visible bounds and inverse rotation around pivots, z-ordered pickSpriteInstances(), and canvas-runtime worldToScreen()/screenToWorld()/pickEntity() integrated with hierarchy/layers/camera/parallax.
