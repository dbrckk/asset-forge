...
- A dependency-free ES module web/runtime_atlas.mjs indexes frames by index/name, draws trim+rotation-correct sprites to Canvas2D, exposes source-oriented UVs for WebGL/custom renderers, indexes animations, samples animation frames deterministically by elapsed seconds, and provides a host-driven play/pause/seek/rate controller with frame/loop/finish/event callbacks plus Canvas2D drawing; event markers remain deterministic across skipped frames and multiple crossed loops. It builds both classic interleaved WebGL sprite batches and compact instanced batches, with a reference WebGL2 shader/attribute contract for drawElementsInstanced, groups instances across multiple texture pages, includes a WebGL2 renderer helper, a centralized reference-counted WebGL2 texture cache with configurable upload settings and deduplicated asynchronous loads, browser fetch/createImageBitmap loading helpers, preload rollback, and a high-level WebGL2 runtime scene that wires loading/cache/batching/rendering/lifecycle together. Scene preparation now supports atlas-aware logical/visible bounds, viewport culling with padding, and stable configurable depth sorting before batching. It also includes HiDPI canvas sizing and a WebGL2 canvas-runtime wrapper that handles context loss/restoration by reconstructing GPU resources and reloading atlas textures. A persistent sprite entity store now supplies retained-mode add/update/remove state, immutable render snapshots, transactional rollback, and direct entity rendering through the canvas runtime while remaining independent of GPU lifecycle. Runtime animations can now bind directly to persistent entities, synchronizing frame/page state and forwarding animation events while remaining independent of GPU context lifecycle. Persistent entities now also support parent-child transform hierarchies resolved into world-space instances before culling, sorting, batching, and rendering. Sprite transforms now also carry world rotation and logical pivots through hierarchy resolution, rotation-aware bounds/culling, classic CPU quads, and the instanced WebGL2 shader. Instanced rendering now also supports per-sprite RGB tint and alpha inherited through entity hierarchies and applied in the fragment shader with standard alpha blending. Scene preparation now also supports 32-bit sprite layers and runtime/per-call visibility masks before culling and batching. Scene preparation now also supports a persistent 2D camera with zoom and per-entity parallax before culling/batching. Camera control now also includes dead-zone camera follow, frame-rate-independent smoothing, and deterministic decaying screen shake. Camera control now also supports zoom-aware world-bounds camera clamping and direct hierarchy-resolved entity follow targets. Interaction helpers now provide world/screen coordinate conversion plus rotation/pivot-aware entity hit testing and z-ordered picking. Interaction now also includes DOM-independent pointer/touch hover, click, capture, drag, and multi-pointer state on top of entity picking. Pointer interaction now also supports hierarchy-safe world-space dragging with optional automatic entity movement and axis constraints. Dragging now also supports world-space snap-to-grid and positional bounds, including automatic reuse of runtime worldBounds.
...
## Current priority
- Harden and productize the retained 2D scene/editor runtime; multi-selection can now be dragged as one hierarchy-safe group with primary-anchored snapping and automatic avoidance of parent+child double movement.
...
## Last verified
- 2026-09-19: latest GitHub source inspected after adding moveSelectedSpriteEntitiesByWorldDelta(), hierarchy-aware selected-root filtering, primary-anchored group snapping, runtime moveSelectionByWorldDelta(), and opt-in pointerOptions.dragSelection for automatic group dragging.
